...
public class DoSReportService implements Reportable {
  ...
  @Override
  public ReportsDTO generate(ReportRequestDTO request)
      throws JAXBException, JsonProcessingException {

    Cases caseEntity = ErrorHandlingUtils.checkEntityExists(
        caseRepository.findById(request.getCaseId()), "Case");

    DosApiResponseDTO dosResponse =
        dosService.getDoSRESTService(createDoSRequest(caseEntity, request.getReferralRequest()));

    return new ReportsDTO(
        mapper.writeValueAsString(createDoSRequest(caseEntity, request.getReferralRequest())),
  ...
  private DoSRequestDTO createDoSRequest(Cases caseEntity, ReferralRequest referralRequest) {
    ...
    dosRequest.setSymptomDiscriminatorInt(
        Integer.valueOf(getServiceRequestedCode(referralRequest, "SD")));
