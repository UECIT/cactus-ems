...
public class GuidanceResponseService {
   ...
   */
  public CdssResult processGuidanceResponse(GuidanceResponse guidanceResponse, Long cdssSupplierId,
      Long caseId) {
  ...
  public void getContained(GuidanceResponse guidanceResponse, Long cdssSupplierId) {
    CdssSupplier cdssSupplier = ErrorHandlingUtils.checkEntityExists(
        cdssSupplierRepo.findById(cdssSupplierId), "CDSS Supplier");
    String baseUrl = cdssSupplier.getBaseUrl();

    if (guidanceResponse.hasResult()) {

      RequestGroup requestGroup = new RequestGroup();
      requestGroup = ResourceProviderUtils
          .getResource(guidanceResponse.getContained(), RequestGroup.class);
      ...
      try {
        requestGroup.getAction().stream().forEach(child -> {
          ...
          Class<? extends Resource> resourceClass = ResourceProviderUtils
              .getResourceType(reference);
          Resource resource = ResourceProviderUtils
              .getResource(fhirContext, baseUrl, resourceClass, reference);
            ...
            childReferences.stream().forEach(childReference -> {
              if (childReference != null) {
                guidanceResponse.addContained(
                    ResourceProviderUtils.getResource(fhirContext, baseUrl,
                        ResourceProviderUtils.getResourceType(childReference), childReference));
              }
            });
          }
    ...
    if (guidanceResponse.hasOutputParameters()) {
      try {
        Parameters parameters = (Parameters) ResourceProviderUtils
            .getResource(fhirContext, baseUrl, Parameters.class,
  ...
  public String getQuestionnaireReference(GuidanceResponse guidanceResponse) {
    if (guidanceResponse.hasDataRequirement()) {
      ...
      if (questionnaireExtensions != null && !questionnaireExtensions.isEmpty()) {
        Reference questionnaireRef = ResourceProviderUtils
            .castToType(questionnaireExtensions.get(0).getValue(),
                Reference.class);
